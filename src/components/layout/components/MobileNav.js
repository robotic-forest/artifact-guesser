            ...
            {user.isLoggedIn ? (
              ...
              <div>
                ...
                <div className='grid grid-cols-2 gap-2 mt-2'>
                  ...
                  <MenuButtonUI onClick={() => { setOpen(false); setSignupOpen(true) }}>
                    Sign Up
                  </MenuButtonUI>
                </div>
              </div>
...
export const MenuButton = ({ url, children, theme: btnTheme, target, style, ...props }) => {
  const styles = btnTheme && css`
    ${themeCSS(btnTheme)}
  ...
  return (
    <Link href={url} target={target}>
      <Button {...props} css={{ ...styles, width: '100%', justifyContent: 'flex-start', ...props.css }} style={style}>
        {children}
      </Button>
    </Link>
  )
...
const MenuButtonUI = ({ children, ...props }) => (
...
const ResponsiveMenu = styled.div`
  ...
  border-left: 1px solid var(--textVeryLowOpacity);
  z-index: 100;
  padding: 12px;
